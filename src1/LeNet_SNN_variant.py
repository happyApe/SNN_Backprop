...
parser.add_argument(
    "--epochs", default=125, type=int, metavar="N", help="number of total epochs to run"
)
...
class CNNModel(nn.Module):
    ...
    def forward(self, input, steps=100, l=0.99):
        ...
        mem_1 = Variable(
            torch.zeros(input.size(0), 20, 28, 28).cpu(), requires_grad=False
        )
        mem_1s = Variable(
            torch.zeros(input.size(0), 20, 14, 14).cpu(), requires_grad=False
        )

        mem_2 = Variable(
            torch.zeros(input.size(0), 50, 14, 14).cpu(), requires_grad=False
        )
        mem_2s = Variable(
            torch.zeros(input.size(0), 50, 7, 7).cpu(), requires_grad=False
        )

        membrane_f0 = Variable(
            torch.zeros(input.size(0), 200).cpu(), requires_grad=False
        )
        membrane_f1 = Variable(torch.zeros(input.size(0), 10).cpu(), requires_grad=True)

        Total_1_output = Variable(
            torch.zeros(input.size(0), 20, 28, 28).cpu(), requires_grad=False
        )
        LF_1_output = Variable(
            torch.zeros(input.size(0), 20, 28, 28).cpu(), requires_grad=False
        )

        Total_2_output = Variable(
            torch.zeros(input.size(0), 50, 14, 14).cpu(), requires_grad=False
        )
        LF_2_output = Variable(
            torch.zeros(input.size(0), 50, 14, 14).cpu(), requires_grad=False
        )

        Total_f0_output = Variable(
            torch.zeros(input.size(0), 200).cpu(), requires_grad=False
        )
        LF_f0_output = Variable(
            torch.zeros(input.size(0), 200).cpu(), requires_grad=False
        )
        ...
        for i in range(steps):
            ...
            # CHECK THIS
            Possion_d_input = (torch.abs(input) / 2 > rand_num).type(
                torch.FloatTensor
            ...
            LF_1_output, Total_1_output, out = LF_Unit(
                l, LF_1_output, Total_1_output, out, out1_temp, i
            )
            ...
            LF_2_output, Total_2_output, out = LF_Unit(
                l, LF_2_output, Total_2_output, out, out2_temp, i
            )
            ...
            LF_f0_output, Total_f0_output, out = LF_Unit(
                l, LF_f0_output, Total_f0_output, out, outf0_temp, i
            )
        ...
        return (
            membrane_f1 / self.fc1.threshold / steps,
            Total_1_output,
            LF_1_output,
            Total_2_output,
            LF_2_output,
    ...
    def tst(self, input, steps=100, l=0.99):
        ...
        with torch.no_grad():
            mem_1 = Variable(
                torch.zeros(input.size(0), 20, 28, 28).cpu(),
                requires_grad=False,
                
            )
            mem_1s = Variable(
                torch.zeros(input.size(0), 20, 14, 14).cpu(),
                requires_grad=False,
                
            )

            mem_2 = Variable(
                torch.zeros(input.size(0), 50, 14, 14).cpu(),
                requires_grad=False,
                
            )
            mem_2s = Variable(
                torch.zeros(input.size(0), 50, 7, 7).cpu(),
                requires_grad=False,
            ...
            membrane_f1 = Variable(
                torch.zeros(input.size(0), 10).cpu(), requires_grad=False,
            )
        ...
        for i in range(steps):
            ...
            Poisson_d_input = ((torch.abs(input) / 2) > rand_num).type(
                torch.FloatTensor
            )
...
def grad_cal(l, LF_output, Total_output):
    ...
    out = LF_output.gt(1e-3).type(torch.FloatTensor) + math.log(l) * torch.div(
        LF_output, Total_output
    )
    return out


def train(
    train_loader, model, criterion, criterion_en, optimizer, epoch, time_steps, leak
):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        (
            ...
            out2_temp,
            outf0_temp,
        ) = model(input_var, steps=time_steps, l=leak)
        ...
        print(
            "Epoch: [{0}] Prec@1 {top1_tr.avg:.3f} Prec@5 {top5_tr.avg:.3f} Entropy_Loss {loss_en.avg:.4f}".format(
                epoch, top1_tr=top1_tr, top5_tr=top5_tr, loss_en=losses_en
            )
        )
...
def validate(val_loader, model, criterion, criterion_en, time_steps, leak):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        print(
            "Test: Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f} Entropy_Loss {losses_en_eval.avg:.4f}".format(
                top1=top1, top5=top5, losses_en_eval=losses_en_eval
            )
        )
...
def main():
    ...
    train_data = dsets.MNIST(
        root="../src/data", train=True, transform=transforms.ToTensor(), download=True
    )
    val_data = dsets.MNIST(
        root="../src/data", train=False, transform=transforms.ToTensor()
    )
    ...
    val_loader = torch.utils.data.DataLoader(
        dataset=val_data, batch_size=int(args.batch_size), shuffle=False
    )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        prec1 = validate(
            val_loader, model, criterion, criterion_en, time_steps=10, leak=0.99
        )
