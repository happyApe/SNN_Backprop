...
parser = argparse.ArgumentParser(description = 'LeNet SNN variant')
parser.add_argument('--dataset', default = 'MNIST',type=str,
					 help = 'dataset = [MNIST]')
parser.add_argument('--start-epoch',default=0,type = int,
					metavar = 'N', help = 'Manual epoch number (useful for restarts)')
...
class SpikingNN(torch.autograd.Function):
    ...
    def forward(self,input):
        ...
        return torch.gt(input,0).type(torch.FloatTensor)
...
def LIF_sNeuron(membrane_potential, threshold, l, i):
    # Check the exceed membrane potential and reset
    ...
    # decay
    ###  Look at this again 
    membrane_potential = 1 * membrane_potential.detach() + membrane_potential - membrane_potential.detach()
    # dividing the spike generate by threshold
...
def Pooling_sNeuron(membrane_potential, threshold, i):
    ...
    # Check exceed potential and reset
    ex_membrane = nn.functional.threshold(membrane_potential, threshold, 0)
...
class CNNModel(nn.Module):
	...
	def forward(self,input, steps = 100, l = 0.99):
			...
			mem_1 = Variable(torch.zeros(input.size(0),20,28,28).cpu(), requires_grad = False)
			mem_1s = Variable(torch.zeros(input.size(0),20,14,14).cpu(),requires_grad = False)

			mem_2 = Variable(torch.zeros(input.size(0), 50, 14, 14).cpu(), requires_grad=False)
			mem_2s = Variable(torch.zeros(input.size(0), 50, 7, 7).cpu(), requires_grad=False)

			membrane_f0 = Variable(torch.zeros(input.size(0), 200).cpu(), requires_grad=False)
			membrane_f1 = Variable(torch.zeros(input.size(0), 10).cpu(), requires_grad=True)

			Total_1_output = Variable(torch.zeros(input.size(0), 20, 28, 28).cpu(), requires_grad=False)
			LF_1_output = Variable(torch.zeros(input.size(0), 20, 28, 28).cpu(), requires_grad=False)

			Total_2_output = Variable(torch.zeros(input.size(0), 50, 14, 14).cpu(), requires_grad=False)
			LF_2_output = Variable(torch.zeros(input.size(0), 50, 14, 14).cpu(), requires_grad=False)

			Total_f0_output = Variable(torch.zeros(input.size(0), 200).cpu(), requires_grad=False)
			LF_f0_output = Variable(torch.zeros(input.size(0), 200).cpu(), requires_grad=False)


			for i in range(steps):
				# Poisson input spike generation 
				rand_num = Variable(torch.rand(input.size(0),input.size(1), input.size(2), input.size(3)).cpu())
				#CHECK THIS
				Possion_d_input = ((torch.abs(input)/2 > rand_num).type(torch.FloatTensor)) # torch.cuda.FloatTensor
				Possion_d_input = torch.mul(Possion_d_input,torch.sign(input))

				# Convolutional Layer
				mem_1 = mem_1 + self.cnn1(Possion_d_input)
	...
	def tst(self,input,steps = 100, l = 0.99):
		...
		mem_1 = Variable(torch.zeros(input.size(0), 20, 28, 28).cpu(), requires_grad=False, volatile=True)
		mem_1s = Variable(torch.zeros(input.size(0), 20, 14, 14).cpu(), requires_grad=False, volatile=True)

		mem_2 = Variable(torch.zeros(input.size(0), 50, 14, 14).cpu(), requires_grad=False, volatile=True)
		mem_2s = Variable(torch.zeros(input.size(0), 50, 7, 7).cpu(), requires_grad=False, volatile=True)

		membrane_f0 = Variable(torch.zeros(input.size(0), 200).cpu(), requires_grad=False, volatile=True)
		membrane_f1 = Variable(torch.zeros(input.size(0), 10).cpu(), requires_grad=False, volatile=True)

		for i in range(steps):
			# Poisson input spike generation
			rand_num = Variable(torch.rand(input.size(0), input.size(1), input.size(2), input.size(3)).cpu())
			Poisson_d_input = ((torch.abs(input)/2) > rand_num).type(torch.FloatTensor)
...
def grad_cal(l, LF_output, Total_output):
	...
	Total_output = Total_output + (Total_output < 1e-3).type(torch.FloatTensor)
	out = LF_output.gt(1e-3).type(torch.FloatTensor) + math.log(l) * torch.div(LF_output, Total_output)
	return out
...
def train(train_loader, model, criterion, criterion_en, optimizer, epoch, time_steps, leak):
	...
	for i, (input, target) in enumerate(train_loader):
		...
		target = target.cpu()
		labels = Variable(target.cpu())
		if device_num < 2:
			input_var = Variable(input.cpu())
		else:
		...
		targetN = output.data.clone().zero_().cpu()
		targetN = Variable(targetN.type(torch.FloatTensor))


		loss = criterion(output, targetN)
		loss_en = criterion_en(output, labels).cpu()
...
def validate(val_loader, model, criterion, criterion_en, time_steps, leak):
	...
	for i, (input, target) in enumerate(val_loader):
		...
		top1.update(prec1, input.size(0))
		top5.update(prec5, input.size(0))
		losses_en_eval.update(loss_en.data, input.size(0))
...
def main():
	...
	criterion = torch.nn.MSELoss(size_average=False).cpu()
	...
	train_data = dsets.MNIST(root='../src/data', train=True, transform=transforms.ToTensor(), download=True)
	val_data = dsets.MNIST(root='../src/data', train=False, transform=transforms.ToTensor())
